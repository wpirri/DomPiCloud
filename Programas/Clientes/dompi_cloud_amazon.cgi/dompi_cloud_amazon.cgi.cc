...
int main(int /*argc*/, char** /*argv*/, char** env)
{
  ...
  if( pConfig->GetParam("TRACE-AMAZON.CGI", s))
  {
    trace = atoi(s);
  }
  ...
  if(trace)
  {
    openlog("dompi_cloud_amazon.cgi", 0, LOG_USER);
  ...
  if(strchr(request_uri, '?'))
  {
    ...
    for(i = 0; Str.ParseDataIdx(get_data, label, value, i); i++)
    {
      ...
      {
        strcpy(funcion_call, "amazon_");
        strcat(funcion_call, value);
      }
    }
  }
  ...
  if(content_length > 0)
  {
    /* Cambio comillas simples por comillas dobles */
    p = &post_data[0];
    while(*p)
    {
      if(*p == '\'') *p = '\"';
    ...
    json_request = cJSON_Parse(post_data);
    if(json_request)
    {
      cJSON_AddItemToObject(json_obj, "request", json_request);
    }
    else
    {
      syslog(LOG_DEBUG, "Error parseando POST data.");
    }
  }
  ...
  if(rc == 0)
  {
    strncpy(buffer, response.Data(), response.Length());
    buffer[response.Length()] = 0;

    /* Cambio comillas doblespor comillas simples */
    //p = &buffer[0];
    //while(*p)
    //{
    //  if(*p == '\"') *p = '\'';
